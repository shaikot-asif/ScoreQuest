...
const MatchStatistics = () => {
  ...
  match?.score?.requestedTeam?.playerStats?.map((item) =>
    console.log(item, "from map")
  );

  console.log(requestingTeamPlayerName, requestedTeamPlayerName, "name");
        ...
        <div className="flex flex-col md:flex-row justify-between items-stretch align-baseline  ">
          ...
          <div className="w-full md:w-[45%] ">
            ...
            >
              ...
              <div className="flex flex-row gap-2 justify-center mt-3">
                <span>Extra Runs: </span>
                <span>lb: {match?.score?.requestedTeam?.extraRun?.legBye}</span>
                <span>b: {match?.score?.requestedTeam?.extraRun?.bye}</span>
                <span>nb: {match?.score?.requestedTeam?.extraRun?.noBall}</span>
                <span>
